  ...
  return (
    <nav className="w-1/4 h-screen flex flex-col bg-emerald-500 shadow-xl fixed">
      ...
      <div className="flex flex-col items-center h-1/2 justify-evenly">
        <h2 className="text-base font-semibold cursor-pointer text-white" onClick={() => navigate('/')}>all blogs.</h2>
        <h2 className="text-base font-semibold cursor-pointer text-white" onClick={() => navigate('/myblogs')}>my blogs.</h2>
        <div className="flex items-center cursor-pointer" onClick={() => navigate('/profile')}>
            <img src={`${baseUrl}${user.profile_img}`} alt="User Profile Image" className="w-7 rounded-full border-2" />
            <h2 className="ml-2 text-base font-semibold text-white">{user.username}.</h2>
        </div>
      </div>
    </nav>
  );
