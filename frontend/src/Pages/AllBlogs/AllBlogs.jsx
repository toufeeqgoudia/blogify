    ...
    <div className="w-3/4 h-full p-5 flex flex-col ml-25">
      ...
      <div>
        {posts.map((post) => (
          ...
          >
            <div className="flex items-center my-1 cursor-pointer" onClick={() => openUserDetailsDialog(post.id)}>
            ...
            <div className="cursor-pointer" onClick={() => openPostDetailsDialog(post.id)}>
              <h4 className="text-lg font-medium">{post.title}<span className="text-xs"> - {formatPostDate(post.date_posted)}</span></h4>
              <p className="text-sm" style={descStyles}>{post.content}</p> 
              <p className="text-xs font-semibold">read more...</p>
            </div>
          </div>
        ))}
      </div>
